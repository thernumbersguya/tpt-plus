...
const LAYERS = ["p", "b", "g", "e", "t", "s", "sb", "sg", "h", "q", "hb", "ss", "hg", "m", "ba", "ps", "sp", "l", "hs", "i", "mt"]
...
const LAYER_RES_CEIL = ["sb", "sg", "hb", "ss", "hs", "hg"]
...
const LAYER_EFFS = {
	b() { 
		...
		return Decimal.pow(Decimal.add(2, tmp.atbb).mul(getRow2Boosts()).max(0), player.b.points.add(getFreeBoosters()).times(getBoosterPower())).max(0)
	},
	g() { return Decimal.pow(Decimal.add(2, tmp.atgb).times(tmp.sGenPowEff).mul(getRow2Boosts()).times((player.ss.upgrades.includes(23) ? LAYER_UPGS.ss[23].currently() : 1)).max(0), player.g.points.times(getGenPow())).sub(1).times(getGenPowerGainMult()).max(0) },
	t() { return {
	...
	sb() { return Decimal.pow(Decimal.add(1.5, addToSBBase()).mul(getRow3Boosts()), player.sb.points.times(getSuperBoosterPow())) },
	sg() { return Decimal.pow(Decimal.add(2, addToSGBase()).mul(getRow3Boosts()).times(tmp.hGenPowEff), player.sg.points).sub(1).times(getSuperGenPowerGainMult()).max(0) },
	h() { 
    if (tmp.challActive?tmp.challActive.mt[12]:true) return new Decimal(0)
	...
	hg() {
    if (tmp.challActive?tmp.challActive.mt[12]:true) return new Decimal(0)
    return Decimal.pow(Decimal.add(2, addToHGBase()), player.hg.points).sub(1).mul(getHyperGenPowerGainMult()).max(0) },
...
const LAYER_UPGS = {
	...
	g: {
		...
		22: {
			...
			unl() { return player.g.upgrades.includes(15) },
			currently() { 
        let ret = player.p.points.add(1).pow(0.25).pow(player.g.upgrades.includes(32)?2.5:1)
        if (player.hg.upgrades.includes(21)) ret = ret.pow(LAYER_UPGS.hg[21].currently())
        return ret
      },
			effDisp(x) { return "/"+format(x) },
		},
	...
	hb: {
		...
		12: {
			...
			unl() { return player.hb.unl },
			currently() { 
        let mult = player.sb.points.div(10).add(1).log10().add(1)
        if (mult.gt(2)) mult = mult.sqrt().mul(Decimal.sqrt(2))
        return mult
      },
			effDisp(x) { return format(x.sub(1).times(100))+"% stronger" },
		},
		...
		15: {
			desc: "Divide Hyper-Generator cost by 1.05.",
			cost: new Decimal(8),
			unl() { return player.hg.upgrades.length > 5 },
		},
	},
	ss: {
		rows: 2,
	...
	hg: {
		...
		cols: 4,
		11: {
			desc: "Super-Generator Power generates faster based on your Hyper-Generators.",
			cost: new Decimal(2),
			unl() { return player.hg.unl },
			currently() { return player.hg.points.sqrt().div(4).add(1).pow(22) },
			effDisp(x) { return format(x)+"x" },
		},
		12: {
			desc: "Hyper-Generator Power generates faster based on your Super-Generators.",
			cost: new Decimal(2),
			unl() { return player.hg.unl },
			currently() { 
        let ret = player.sg.points.div(10).add(1).log10().add(1).pow(1.5)
        if (ret.gt(1e15)) ret = ret.add(1).log10().mul(Decimal.log10(1e15).add(1))
        return ret },
			effDisp(x) { return format(x)+"x" },
		},
		13: {
			desc: "Hyper-Generators boost Quirk Layer efficiency.",
			cost: new Decimal(2),
			unl() { return player.hg.upgrades.length > 1 },
			currently() { 
        let ret = player.hg.points.sqrt().div(2).add(1)
        if (ret.gt(1.5)) ret = ret.sqrt().mul(1.59)
        return ret },
			effDisp(x) { return format(x.sub(1).mul(100))+"%" },
		},
		14: {
			desc: "Unlock a new Life Booster.",
			cost: new Decimal(3),
			unl() { return player.hg.upgrades.length > 2 },
		},
		21: {
			desc: "Raise the power of the seventh Generator Upgrade based on Hyper-Generator Power.",
			cost: new Decimal(4),
			unl() { return player.hg.upgrades.length > 3 },
			currently() { 
        let mult = player.hg.power.div(10).add(1).log10().add(1).cbrt()
        if (mult.gt(3)) mult = mult.add(1).log10().add(1).log10().add(1).mul(3)
        return mult
      },
			effDisp(x) { return "^"+format(x) },
		},
		22: {
			desc: "Reduce the Super-Booster cost based on Hyper-Generator Power.",
			cost: new Decimal(5),
			unl() { return player.hg.upgrades.length > 4 },
			currently() { return player.hg.power.add(1).log10().div(4).add(1).pow(15) },
			effDisp(x) { return "/"+format(x) },
		},
		23: {
			desc: "Hyper-Generators make Generator Power more effective.",
			cost: new Decimal(6),
			unl() { return player.hg.upgrades.length > 5 },
			currently() { return player.hg.points.add(1).log10().div(4).add(1).root(1.5) },
			effDisp(x) { return format(x.sub(1).mul(100))+"% stronger" },
		},
		24: {
			desc: "Hyper-Generators raise prestige point gain.",
			cost: new Decimal(7),
			unl() { return player.hg.upgrades.length > 5 },
			currently() { return player.hg.points.add(1).log10().div(4).add(1).root(2.75) },
			effDisp(x) { return "^"+format(x) },
		},
	},
	m: {
	...
	mt: {
		...
		cols: 5,
		11: {
			desc: "Meta Points multiply Point gain.",
			cost: new Decimal(5),
			unl() { return player.mt.unl },
			currently() { return player.mt.points.add(1).pow(1e5) },
			effDisp(x) { return format(x)+"x" },
		},
		12: {
			desc: "Points multiply Prestige Point gain once more.",
			cost: new Decimal(15),
			unl() { return player.mt.upgrades.length > 0 },
			currently() { return player.points.add(1).tetrate(0.01) },
			effDisp(x) { return format(x)+"x" },
		},
		13: {
			desc: "Multiply Booster and Generator bases based on best Meta Points.",
			cost: new Decimal(30),
			unl() { return player.mt.upgrades.length > 1 },
			currently() { return player.mt.best.add(1).pow(15) },
			effDisp(x) { return format(x)+"x" },
		},
		14: {
			desc: "Super-Generators boost Meta Points.",
			cost: new Decimal(45),
			unl() { return player.mt.upgrades.length > 2 },
			currently() { return player.sg.points.add(1).log10().add(1).log10().add(1) },
			effDisp(x) { return format(x)+"x" },
		},
		15: {
			desc: "Row 3 super-item base is multiplied by Meta Points.",
			cost: new Decimal(50),
			unl() { return player.mt.upgrades.length > 3 },
			currently() { return player.mt.points.add(1).log10().add(1).log10().add(1).pow(3) },
			effDisp(x) { return format(x)+"x" },
		},
		21: {
			desc: "Quirk Layers are boosted by Meta Points.",
			cost: new Decimal(200),
			unl() { return player.mt.upgrades.length > 4 },
			currently() { 
        let ret = player.mt.points.add(1).pow(3)
        if (ret.gt(1e30)) ret = ret.sqrt().mul(Decimal.sqrt(1e30))
        return ret },
			effDisp(x) { return format(x)+"x" },
		},
		22: {
			desc: "Imperium Buildings boost Subspace gain.",
			cost: new Decimal(300),
			unl() { return player.mt.upgrades.length > 5 },
			currently() { return player.i.extraBuildings.add(1).pow(5).add(1) },
			effDisp(x) { return format(x)+"x" },
		},
		23: {
			desc: "Phantom Soul cost is divided by Hyper-Generators.",
			cost: new Decimal(1e7),
			unl() { return player.mt.upgrades.length > 6 },
			currently() { return player.hg.points.add(1).pow(1e6).add(1) },
			effDisp(x) { return "/"+format(x) },
		},
		24: {
			desc: "Life Brick and Imperium Brick gain are 50% better",
			cost: new Decimal(5e7),
			unl() { return player.mt.upgrades.length > 7 },
		},
		25: {
			desc: "The Life Essence gain is multiplied based on Life Essence",
			cost: new Decimal(5e8),
			unl() { return player.mt.upgrades.length > 8 },
			currently() { return player.l.points.add(1).cbrt().tetrate(0.1).log10().add(1).tetrate(0.1).cbrt() },
			effDisp(x) { return format(x)+"x" },
		},
  }
}
...
function getLayerEffDesc(layer) {
	...
	switch(layer) {
		...
		case "hb": 
			...
			break;
		case "hg":
			return "which are generating "+format(eff)+" Hyper-Generator Power/sec"
			break;
		case "ss": 
...
function checkForVars() {
  ...
  if (player.hg === undefined) {
    player.hg = start.hg
  }
  if (player.mt === undefined) {
    player.mt = start.mt
  }
}
...
function convertToDecimal() {
	...
	for (let i=1;i<=8;i++) if (player.l.boosters[i]) player.l.boosters[i] = new Decimal(player.l.boosters[i])
  ...
  player.hg.power = new Decimal(player.hg.power)
}
...
function getLayerGainMult(layer) {
	...
	switch(layer) {
		case "p":
			...
			if (player.mt.upgrades.includes(12)) mult = mult.times(LAYER_UPGS.mt[12].currently())
			break;
		case "b": 
		...
		case "sb":
			...
			if (player.hg.upgrades.includes(22)) mult = mult.div(LAYER_UPGS.hg[22].currently())
			break;
		case "sg": 
		...
		case "hb": 
			...
			break;
		case "hg": 
			if (player.hb.upgrades.includes(15)) mult = mult.div(1.05)
			break;
		case "h": 
		...
		case "ba": 
			...
			break;
		case "ps": 
			if (player.mt.upgrades.includes(23)) mult = mult.div(LAYER_UPGS.mt[23].currently())
		...
		case "l": 
			...
			if (player.mt.upgrades.includes(25)) mult = mult.times(LAYER_UPGS.mt[25].currently())
			break;
		case "hs": 
			...
			break;
		case "mt": 
			if (player.mt.upgrades.includes(14)) mult = mult.times(LAYER_UPGS.mt[14].currently())
			if (player.mt.challs.includes(11)) mult = mult.times(LAYER_CHALLS.mt[11].currently())
			break;
	}
	return mult
}
...
function getLayerGainExp(layer) {
	...
	switch(layer) {
		case "p": 
			...
			if (player.hg.upgrades.includes(24)) exp = exp.times(LAYER_UPGS.hg[24].currently())
			break;
		case "ps": 
		...
		case "hs":
			...
			break;
		case "i": 
			if (player.mt.upgrades.includes(24)) exp = exp.mul(1.5)
			break;
	}
	return exp;
}
...
function getResetGain(layer) {
	...
	if (LAYER_TYPE[layer]=="static") {
		...
		if ((LAYER_ROW[layer] < 4 && layer != "hb") || layer == "ps") {
			...
			if (gain.gte(1900)) gain = gain.times(Decimal.pow(1900, 9)).pow(0.1)
		}
...
function getNextAt(layer) {
	...
	if (LAYER_TYPE[layer]=="static") {
		...
		if ((LAYER_ROW[layer] < 4 && layer != "hb" && !(layer == "hg")) || layer == "ps") {
			if (amt.gte(1900)) amt = amt.pow(10).div(Decimal.pow(1900, 9))
...
function rowReset(row, layer) {
	switch(row) {
		...
		case 6:
			...
			if (!player.mt.best.gte(1)) player.sp = start.sp
			if (!player.mt.best.gte(2)) player.l = start.l
			if (!player.mt.best.gte(3)) player.hs = start.hs
			if (!player.mt.best.gte(4)) player.i = start.i
      let lsit = ["l", "hs", "i"]
      for (var i in lsit) {
        player[lsit[i]].unl = true
      }
			break;
	}
}
...
function getPointGen() {
	...
	if (player.mt.upgrades.includes(11)) gain = gain.times(LAYER_UPGS.mt[11].currently())
		
	if (tmp.challActive ? tmp.challActive.h[31] : true) gain = gain.tetrate(0.1)
	if (tmp.challActive ? tmp.challActive.mt[11] : true) gain = gain.tetrate((5+player.h.challs.length)/1e6)
	return gain
}
...
function getGenPowerEffExp() {
	...
	if (player.hg.upgrades.includes(23)) exp = exp.mul(LAYER_UPGS.hg[23].currently())
	return exp;
}
...
function getRow2Boosts() {
  let mult = new Decimal(1)
	if (player.mt.upgrades.includes(13)) mult = mult.times(LAYER_UPGS.mt[13].currently())
  return mult
}
...
function getRow3Boosts() {
  let mult = new Decimal(1)
	if (player.mt.upgrades.includes(15)) mult = mult.times(LAYER_UPGS.mt[15].currently())
  return mult
}
...
function getQuirkLayerMult() {
	...
	if (player.mt.upgrades.includes(21)) mult = mult.times(LAYER_UPGS.mt[21].currently())
	return mult
}
...
function getQuirkEnergyGainExp() {
  ...
  if (player.hg.upgrades.includes(13)) mult = mult.times(LAYER_UPGS.hg[13].currently())
	return player.q.layers.add(getExtraQuirkLayers()).sub(1).times(mult)
}

function getQuirkEnergyEff() {
  if (tmp.challActive?tmp.challActive.mt[12]:true) return new Decimal(1)
...
const LAYER_CHALLS = {
	...
	mt: {
		...
		cols: 2,
		11: {
			name: "Tetrational Hazard",
			desc: "Point gain is tetrated based on x (5 plus hindrance challenge completions) divided by 1e6",
			unl() { return player.mt.upgrades.length > 9 },
			goal: new Decimal("e355000"),
			reward: "Points boost meta point gain",
			currently() { 
				let ret = player.points.add(1).sqrt().log10().add(1).tetrate(0.01)
				return ret;
			},
			effDisp(x) { return format(x)+"x" },
		},
		12: {
			name: "Skip the Fourth",
			desc: "Row 4 is disabled (but their upgrades are not).",
			unl() { return player.mt.upgrades.length > 9 },
			goal: new Decimal("e1.5e9"),
			reward: "Hyper-Generator Power boosts itself.",
			currently() { 
				let ret = player.hg.power.add(1).sqrt().log10().add(1)
				return ret;
			},
			effDisp(x) { return format(x)+"x" },
		},
...
function startChall(layer, x) {
    ...
    if (layer == "mt") {
      player.h.challs = [11, 12, 21, 22, 31, 32, 41, 42, 51, 52, 61, 62, 71]
    }
  ...
  if (layer == "mt" && player[layer].active != undefined) {
    player.h.challs = []
  }
	doReset(layer, true)
...
function getSubspaceGainMult() {
	...
	if (player.mt.upgrades.includes(22)) mult = mult.times(LAYER_UPGS.mt[22].currently())
	return mult
}
...
function getHGenPowEff() {
	if (!player.hg.unl) return new Decimal(1)
	let eff = player.hg.power.add(1).pow(3)
	return eff
}
...
function getSuperGenPowerGainMult() {
	...
	if (player.hg.upgrades.includes(11)) mult = mult.times(LAYER_UPGS.hg[11].currently())
	return mult
}
...
let LIFE_BOOSTERS = {
	max: 8,
	unl() {
		if (player.ps.points.gte(86)) return 8
		if (player.hg.upgrades.includes(14)) return 7
		if (player.hg.unl) return 6
		if (player.ps.upgrades.includes(21)) return 5
		return 4
	},
	calcNewPower(diff) {
		if (!player.l.unl || player.l.points.lt(1)) return new Decimal(0)
	...
	eff(x) {
    let mul = player.l.power.add(1).log10()
  	if ((player.l.unl && tmp.l !== undefined && tmp.l.lbUnl >= 8) && x<8) mul = mul.times(Decimal.add(1, tmp.l.lbEff[8]))
  
    if (mul.gt(2500)) mul = mul.root(9).mul(Decimal.pow(2500, 8/9))
		return mul
	},
	req(x) {
    let mul = tmp.l.lb[x].times(this[x].reqMult).add(this[x].req);
		return mul
	},
	reqTarget(x) {
	...
	6: {
		req: new Decimal(30),
		reqMult: new Decimal(13),
		eff(str) {
			return Decimal.pow(1.12, str.sqrt())
		},
		effDesc(x) {
			return "Gain " + format(x) + "x more Hyper-Generator Power"
		}
	},
	7: {
		req: new Decimal(55),
		...
		eff(str) {
			return str.add(1).log10().pow(1.25)
		},
		effDesc(x) {
			return "Subtract " + formatWhole(x) + " Life Bricks from the Imperium Building cost"
		}
	},
	8: {
		req: new Decimal(76),
		reqMult: new Decimal(121),
		eff(str) {
			return str.root(10)
		},
		effDesc(x) {
			return "Boost all previous Life Boosters by " + format(x) + "%"
		}
	},
}
...
let HYPERSPACE = {
	...
	effs: {
		1(sb, su) {
      if (su.gt(3)) su = su.div(10).add(3.9)
...
let IMPERIUM = {
	lifeReq() {
    let req = Decimal.pow(10, player.i.lifeBricks.times(2).sqr().add(15))
    if (player.mt.upgrades.includes(24)) req = req.root(1.5)
		return req
	},
	lifeTarget() {
    let req = player.l.power.log10().sub(15).sqrt().div(2).add(1).floor()
    if (player.mt.upgrades.includes(24)) req = req.pow(1.5).floor()
		return req
	},
	canBuild() {
		let cost = this.cost()
    ...
    let lifeSub = tmp.l.lb[7] ? tmp.l.lbEff[7] : 0
    let scaling = x.sub(20).max(1).pow(2)
		return {
			i: x.times(1.75).add(0.5).mul(scaling).sub(sub).ceil(),
			l: x.times(1.5).add(1).mul(scaling).sub(sub).sub(lifeSub).ceil()
		}
	},
	speed() {
		let x = Decimal.pow(1.05, player.i.extraBuildings.add(5)).recip()
...
function getHyperGenPowerGainMult() {
	let mult = new Decimal(1)
	if (player.hg.upgrades.includes(12)) mult = mult.times(LAYER_UPGS.hg[12].currently())
	if (player.l.unl && tmp.l !== undefined && tmp.l.lbUnl >= 6) mult = mult.times(tmp.l.lbEff[6])
	if (player.mt.challs.includes(12)) mult = mult.times(LAYER_CHALLS.mt[12].currently())
	return mult
}

const ENDGAME = new Decimal("ee13");
...
function gameLoop(diff) {
	...
	if (player.hg.unl) player.hg.power = player.hg.power.add(tmp.layerEffs.hg.times(diff)).max(0)
	if (player.ba.unl) {
	...
	if (player.mt.best.gte(100)) {
    generatePoints("sp", diff)
    generatePoints("l", diff)
    generatePoints("hs", diff)
  }
